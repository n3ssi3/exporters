...
function shouldDisableDefaultTailwindConfiguration(): boolean {
  return (
    exportConfiguration.disableAllDefaults ||
...
export function styleOutputFile(
  ...
  const hasReferences =
    exportConfiguration.useReferences &&
  ...
  content += generateTypographyUtility(processedTokens, tokenGroups)
  ...
  return Array.from(tokensByType.entries())
    .map(([type, tokensOfType]) => {
      ...
      if (type === TokenType.typography) {
        ...
        content += generateTypographyUtility(tokensOfType, tokenGroups)
      }
...
export function indexOutputFile(tokens: Array<Token>, themes: Array<TokenTheme | string> = []): OutputTextFile | null {
  ...
  if (
    exportConfiguration.fileStructure === FileStructure.SeparateByType &&
    shouldDisableDefaultTailwindConfiguration()
  ) {
  ...
  if (exportConfiguration.exportBaseValues) {
    if (exportConfiguration.fileStructure === FileStructure.SingleFile) {
      ...
      const tokenTypes = Array.from(new Set(tokens.map((t) => t.tokenType))).filter((type) => isAllowedTokenType(type))
